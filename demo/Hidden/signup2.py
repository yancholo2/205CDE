...
def index():
	if "username" in session:
		return 'Logged in as' + '<td>' +session["username"] + '<br>' + "<a href = '/logout'>Click Here to log out</a>"
	else:
		return "Hello ! You are not logged in <br><a href= '/login'>Click here to log in</a>"
...
def signup():
	...
	if request.method == 'POST':
		...
		if usrname == custName[0]:
			return render_template("signup.html",test="Username used")
		else:
			sql = ("""INSERT INTO user (username, password) VALUES (%s, %s)""", (usrname, pwd))
			cursor.execute(sql)
...
def payment():
	...
	if request.method == 'POST':
		...
		cursor.execute("""INSERT INTO pay (card, month, year, code) VALUES (%s, %s, %s, %s)""", (CN, MM, YY, CV))
